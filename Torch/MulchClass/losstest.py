...
#train_dataset= dataload.Twostream(annotation_train,still_dir, optic_dir, classes, transform_train)
#test_dataset = dataload.Twostream(annotation_test, still_dir, optic_dir, classes, transform_test)
...
def train(train_loader, learning_rate):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        outputs = model(images_x, images_y, sound)
        #loss = 1*F.multilabel_soft_margin_loss(outputs, labels.cuda(non_blocking=True).float())
        ## outputs - labeles の torch.FloatTensor.abs_() 絶対値
        loss = torch.FloatTensor.abs_(torch.add(-outputs, labels.cuda(non_blocking=True).float()))
        loss = loss*loss
        loss = loss.sum()/len(labels[0])
        # print(lloss.log())
        # print(torch.exp(lloss))
        # print(lloss.sin())
        # print(loss)
        loss.backward()
        optimizer.step()
...
def test(test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(test_loader):
            ...
            outputs = model(images_x, images_y, sounds)
            #loss = 1*F.multilabel_soft_margin_loss(outputs, labels.cuda(non_blocking=True).float())
