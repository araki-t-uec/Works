...
model_image.classifier[6] = nn.Linear(4096, classes_num)
model_sound = networks.ArakiSoundNet()

#networks.SoundBased_ThreestreamNet()
if opt.mulch_gpu == "True":
    print("Let's use", torch.cuda.device_count(), "GPUs!")
    model_image = nn.DataParallel(model_image) # make parallel
...
def train(train_loader, learning_rate):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        outputs_image = model_image(images)
        outputs_sound = model_sound(sounds)
        outputs = (outputs_image + outputs_sound )/2
...
if optim == "sgd":
    ...
    optimizer = torch.optim.SGD(model_image.parameters(), lr=learning_rate, momentum=0.9)    
    optimizer = torch.optim.SGD(model_sound.parameters(), lr=learning_rate, momentum=0.9)    
else:
    optimizer = torch.optim.Adam(model_image.parameters(), lr=learning_rate)
...
for epoch in range(epochs):
    ...
    ### Save a model.
    if val_loss < oldloss:
        torch.save(model_image.state_dict(), os.path.join(result_path+corename+'_image.ckpt'))
